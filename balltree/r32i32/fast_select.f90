...
CONTAINS
  ...
  !   VALUES(K) has rank K.
  ! 
  RECURSIVE SUBROUTINE ARGSELECT_R32(VALUES, INDICES, K, DIVISOR, MAX_SIZE)
    ! Arguments
    ...
    DO WHILE (LEFT .LT. RIGHT)
       ...
       IF (RIGHT - LEFT .GT. MS) THEN
          ...
          CALL ARGSELECT_R32(VALUES(L:R), INDICES(L:R), K - L + 1, DIVISOR, MAX_SIZE)
       END IF
       ...
       CALL SWAP_R32(VALUES(LEFT), VALUES(K))
       CALL SWAP_I32(INDICES(LEFT), INDICES(K))
       ...
       DO WHILE (L .LT. R)
          CALL SWAP_R32(VALUES(L), VALUES(R))
          CALL SWAP_I32(INDICES(L), INDICES(R))
          L = L + 1
          R = R - 1
       ...
       IF (VALUES(LEFT) .EQ. P) THEN
          CALL SWAP_R32(VALUES(LEFT), VALUES(R))
          CALL SWAP_I32(INDICES(LEFT), INDICES(R))
       ELSE
          R = R + 1
          CALL SWAP_R32(VALUES(R), VALUES(RIGHT))
          CALL SWAP_I32(INDICES(R), INDICES(RIGHT))
       END IF
       ...
       IF (K .LE. R) RIGHT = R - 1
    END DO
  END SUBROUTINE ARGSELECT_R32

  ! ------------------------------------------------------------------

  RECURSIVE SUBROUTINE ARGSELECT_I32(VALUES, INDICES, K, DIVISOR, MAX_SIZE)
    ! Arguments
    INTEGER(KIND=INT32), INTENT(INOUT), DIMENSION(:) :: VALUES
    INTEGER(KIND=INT32), INTENT(INOUT), DIMENSION(:) :: INDICES
    INTEGER(KIND=INT32), INTENT(IN)                  :: K
    INTEGER(KIND=INT32), INTENT(IN), OPTIONAL        :: DIVISOR, MAX_SIZE
    ! Locals
    ...
    DO WHILE (LEFT .LT. RIGHT)
       ...
       IF (RIGHT - LEFT .GT. MS) THEN
          ...
          R = L + (SIZE(VALUES) / D)
          ! Perform fast select on an array a fraction of the size about K.
          CALL ARGSELECT_I32(VALUES(L:R), INDICES(L:R), K - L + 1, DIVISOR, MAX_SIZE)
       END IF
       ! Pick a partition element at position K.
       P = VALUES(K)
       L = LEFT
       R = RIGHT
       ! Move the partition element to the front of the list.
       CALL SWAP_I32(VALUES(LEFT), VALUES(K))
       CALL SWAP_I32(INDICES(LEFT), INDICES(K))
       ! Pre-swap the left and right elements (temporarily putting a
       ! larger element on the left) before starting the partition loop.
       IF (VALUES(RIGHT) .GT. P) THEN
          CALL SWAP_I32(VALUES(LEFT), VALUES(RIGHT))
          CALL SWAP_I32(INDICES(LEFT), INDICES(RIGHT))
       END IF
       ! Now partition the elements about the pivot value "T".
       DO WHILE (L .LT. R)
          CALL SWAP_I32(VALUES(L), VALUES(R))
          CALL SWAP_I32(INDICES(L), INDICES(R))
          L = L + 1
          R = R - 1
          DO WHILE (VALUES(L) .LT. P) ; L = L + 1 ; END DO
       ...
       ! Place the pivot element back into its appropriate place.
       IF (VALUES(LEFT) .EQ. P) THEN
          CALL SWAP_I32(VALUES(LEFT), VALUES(R))
          CALL SWAP_I32(INDICES(LEFT), INDICES(R))
       ELSE
          R = R + 1
          CALL SWAP_I32(VALUES(R), VALUES(RIGHT))
          CALL SWAP_I32(INDICES(R), INDICES(RIGHT))
       END IF
       ! adjust left and right towards the boundaries of the subset
       ! containing the (k - left + 1)th smallest element
